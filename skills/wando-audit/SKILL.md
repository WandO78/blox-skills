...
!`ls -d plans/ completed/ docs/ .claude/ .github/ src/ app/ test/ tests/ __tests__/ 2>/dev/null`
...
**Summary:** X/19 PRESENT, Y/19 PARTIAL, Z/19 MISSING
...
|----------------|-------------------|
| Gaps found → retrofit needed | "Run `/wando:init` in retrofit mode with this report as input" |
| Quality baseline low | "Run `/wando:review` for detailed code quality assessment" |
| Docs stale | "Run `/wando:gc` for documentation cleanup" |
...
- Gap Analysis table with status for all 19 standard elements (including G-18 AI attribution check, G-19 phase marker consistency)
...
- Gap analysis incomplete (fewer than 19 elements checked)
