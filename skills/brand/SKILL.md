...
> basic mode; enhanced with the Brand Voice plugin when available. Brand produces the
> brand spec/guidelines — actual logo and image generation is handled by `/blox:media`.
...
| Logo/assets | Text spec/description (generate via `/blox:media`) | Same — richer spec, then `/blox:media` generates |

**Logo & image assets:** Brand defines the logo/asset *spec* (concept direction, usage
rules, color/typography constraints). The actual logo and image generation is done by
`/blox:media`, which shows cost before any paid generation. Route the user there when
they want real assets produced — see SKILL INTEGRATIONS.
...
**Output format — dual format (CSS `:root` + Tailwind config fragment, with dark-mode
`@media` overrides).** Use the exact template in `references/token-formats.md` (Step 2).
...
**Output format:** `brand-typography.css` with `--font-*`, `--text-*`, and `--leading-*`
custom properties. Use the exact template in `references/token-formats.md` (Step 3).
...
Also generate a JSON version for programmatic access (`design-tokens.json` with
`colors`/`typography`/`spacing`/`radius`/`shadows`/`breakpoints` keys) — see
`references/token-formats.md` (Step 5).
...
Three worked end-to-end walkthroughs (restaurant autopilot, tech-startup standalone,
archetype-fallback) → see `references/examples.md`.

---

## REFERENCES

- `references/examples.md` — 3 worked end-to-end brand walkthroughs
- `references/token-formats.md` — verbatim CSS / Tailwind / JSON output templates (Steps 2, 3, 5)
